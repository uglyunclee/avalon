...
# [已優化] 補上 2, 3, 4 人的測試設定 (好人, 壞人)
BALANCE_CONFIG = {
    1: (1, 0),
    2: (1, 1), 3: (2, 1), 4: (3, 1),  # 測試用配置
    5: (3, 2), 6: (4, 2), 7: (4, 3),
    8: (5, 3), 9: (6, 3), 10: (6, 4)
}
...
QUEST_CONFIG = {
    1: [1, 1, 1, 1, 1],
    2: [1, 1, 1, 1, 1], 3: [1, 2, 1, 2, 2], 4: [2, 2, 2, 3, 3],  # 測試用
...
async def join_room(sid, data):
    ...
    if token and token in room['players']:  # 重連
        ...
        room['players'][new_token] = {'token': new_token, 'name': name, 'avatar': avatar, 'sid': sid, 'role': None,
...
async def start_game_logic(room_id):
    ...
    settings = room['settings']
    target_good, target_evil = BALANCE_CONFIG.get(cnt, (1, 0))  # 若人數不在表內，預設 (1,0)
...
async def request_reset(sid, room_id):
    ...
    if token not in room['reset_votes']:
        ...
        await add_log(room_id, f"⚠️ 請求重置 ({len(room['reset_votes'])}/{len(room['players'])})", "orange")
