...
async def broadcast_state(room_id):
    ...
    data = {
        ...
        'revealed_roles': revealed_roles # 傳送身分表
    }
...
async def start_game_logic(room_id):
    ...
    room['reset_votes'] = set(); room['state'] = GameState.TEAM_SELECTION
...
async def vote_team(sid, data):
    ...
    if len(room['votes']) == active_players_count:
        ...
        history_entry = { 'quest': room['quest_index'] + 1, 'leader': leader_name, 'team': [room['players'][t]['name'] for t in room['current_team']], 'votes': {room['players'][t]['name']: v for t, v in room['votes'].items()}, 'result': '通過' if passed else '否決', 'mission_result': None, 'fail_count': 0 }
